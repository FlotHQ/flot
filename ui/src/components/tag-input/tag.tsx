...
import { type BaseFieldProps, BaseFieldWrapper } from "~/pages/workflows/fields/base";
...
export function TagInput({ type, ...props }: Props) {
  ...
  const editor = useEditor({
    ...
    onBlur: ({ event }) => {
      if (fieldRef.ref) {
        if (
          event.relatedTarget &&
          (fieldRef.ref.current?.contains(event.relatedTarget as Node) || fieldRef.ref.current === event.relatedTarget)
        ) {
  ...
  useEffect(() => {
    ...
    if (editor && fieldRef.selectedField?.fieldKey === props.id && fieldRef.selectedField?.nodeId === props.id) {
      fieldRef.setEditor(editor);
    }
  ...
  return (
    <BaseFieldWrapper id={`${props.fieldKey}-${props.nodeId}`} {...props}>
      <EditorContent
        ...
        className={cn(tagInputVariants({ type }), isError && "border-destructive")}
        editor={editor}
      />
    </BaseFieldWrapper>
  );
