    ...
    <NodeViewWrapper className="react-component mx-[2px]" data-drag-handle>
      <div
        className={cn(
          ...
          isError ? "border-red-600 bg-red-700 text-white" : "text-background bg-foreground"
        )}
      >
        ...
        <X
          ...
          className={cn("h-3 w-3 mt-[2px] hover:cursor-pointer", isError ? "text-white" : " text-background")}
        />
      </div>
    </NodeViewWrapper>
  );
    ...
    return {
      ...
      renderHTML({ options, node }) {
        return [
          "ref",
          {
            ...Object.fromEntries(Object.entries(node.attrs).map(([key, value]) => [`data-${key}`, value])),
          },
