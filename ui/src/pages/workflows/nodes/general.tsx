...
import { Popover, PopoverTrigger, PopoverContent, PopoverAnchor, PopoverArrow } from "~/components/ui/popover";
    ...
    <Popover modal>
      <PopoverTrigger>
        <div className="relative ">
          ...
          <Card className={cn("hover:cursor-pointer z-50  rounded-lg   group relative w-[300px] border-[1px] p-0 ")}>
            <CardHeader className="px-3 py-4 pb-4 h-full relative ">
              ...
              <div style={{ margin: 0 }} className="pl-2 h-full flex justify-between relative ">
      ...
      <FieldsPanel input={input} nodeId={props.id} fields={fields} handleFieldChange={handleFieldChange} />
    </Popover>
  );
}
...
function FieldsPanel(props: FieldsPanelProps) {
  ...
  return (
    <PopoverContent
      align="center"
      autoFocus={false}
      collisionBoundary={reactflowPaneRef.current ? [reactflowPaneRef.current] : []}
      side="right"
      ref={fieldPanelRef}
        ...
        <div className="space-y-3 px-[1px] pb-8">
          <ViewStackProvider>
            {props.fields.map((field) => (
              <Popover key={field.key} open={!!selectedField && selectedField.fieldKey === field.key}>
                <PopoverAnchor>
                  <Field
                    {...field}
                    key={field.key}
                    ...
                    onValueChange={(value: unknown) => props.handleFieldChange(field.key, value)}
                    label={field.label}
                    fieldKey={field.key}
                  />
                </PopoverAnchor>
                <PopoverPrimitive.Portal container={fieldPanelRef.current}>
                  <PopoverPrimitive.Content
                    ...
                    alignOffset={18}
                    collisionBoundary={
                      reactflowPaneRef.current ? [reactflowPaneRef.current, fieldPanelRef.current] : []
                    }
                    collisionPadding={{
                      left: -200,
                    }}
