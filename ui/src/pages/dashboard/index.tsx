					...
					<div className="grid grid-cols-2 gap-4 text-sm">
						...
						{execution.retries && (
							<div>
								...
								<p>{execution.retries.count} of {execution.retries.max}</p>
							</div>
						)}
...
function RecentExecutions() {
	...
	return (
		<section className="rounded-lg border px-5 pt-5 h-full">
			...
			<ScrollArea className="h-[calc(100%-3rem)] -mr-3">
		...
		<Card className={cn("transition-all duration-200 rounded-md")}>
			<CardHeader className="p-3 pb-2">
				...
				<div className="flex justify-between items-center">
					...
					<span className={cn("text-xs capitalize", statusColors[workflow.status])}>
						{workflow.status}
					</span>
				</div>
			</CardHeader>
...
function RecentWorkflowsPanel() {
	return (
		<section className="rounded-lg border px-4 pt-4 h-full">
			...
			<ScrollArea className="h-[calc(100%-2.5rem)] -mr-3">
			...
			>
				<CarouselContent className="-ml-4">
					{relatedTemplates.map((template) => (
						<CarouselItem key={template.id} className="pl-4 basis-1/2">
							<Card className="transition-all duration-200 hover:border-primary/20 h-[220px] flex flex-col">
